...
- `monitoring-tab-api-keys` 显示 Keys 数据页签（完整文案为 Client Keys，AX 名通常为短名 `Keys`）。
...
- **Hash 入口。** 打开 `#/demo/monitoring`。执行 `control-cpamp goto --hash '#/demo/monitoring'`。证明用户入口时优先用导航点击。路由稳定后可见 `Accounts`、`Keys`、`Realtime`。
- **Accounts 页签。** 选择 `Accounts`。执行 `control-cpamp click --role tab --name 'Accounts'`。账户概览内容仍可见。
- **Keys 页签。** 选择 `Keys`（完整文案可能是 Client Keys）。执行 `control-cpamp click --role tab --name 'Keys'`。Client key 分析内容出现。
...
- 页签 accessible name 可能带徽章，例如 `Realtime 367`。用不精确名称匹配 `Realtime`、`Accounts` 或 `Keys`。
...
- 窄屏时页签文案会缩短。宽屏完整名可能是 `Client Keys`，但 demo 默认 AX 名是短名 `Keys`；配方用 `Keys`。
